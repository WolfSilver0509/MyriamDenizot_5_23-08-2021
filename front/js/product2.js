...
fetch(objectURL)
    ...
    .then(data => {
        ...
        for (i = 0; i < data.colors.length; i++) {
            ...
            localStorage.setItem('printPrice', JSON.stringify(data.price));
            localStorage.setItem('printName', JSON.stringify(data.name));
            localStorage.setItem('printDescription', JSON.stringify(data.description));
            localStorage.setItem('printImg', JSON.stringify(data.imageUrl));
            localStorage.setItem('printAltTxt', JSON.stringify(data.altTxt));
...
function getColor() {
  ...
  sendBtnProductCart.addEventListener("click", () => {
    ...
    const printPrice = localStorage.getItem('printprice');
    const printName = localStorage.getItem('printName');
    const printDescription = localStorage.getItem('printDescription');
    const printImg = localStorage.getItem('printImg');
    const printAltTxt = localStorage.getItem('printAltTxt');
    
    
    let product = {id, color, quantity, printPrice, printName, printImg, printDescription,printAltTxt };
    let productS = [];
